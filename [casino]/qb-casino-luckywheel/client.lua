	...
	while true do
        ...
        if distance < 2.9 and not _isRolling then
            if distance < 2.8 and not _isRolling then
                inZone  = true
				text = "<b>Diamond Casino lykkehjul</b></p> "..Config.startingPrice.." DKK for et spin"
            end
	...
	if Config.CheckMembership then
		QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
			if HasItem then
				...
				QBCore.Functions.Notify('Du har ikke '..Config.CasinoMembership..', det skal du have', 'error', 3500)
			end
...
AddEventHandler("dojLuckywheel:winCar", function() 
	...
	QBCore.Functions.SpawnVehicle(Config.VehiclePrize, function(veh)
		...
		QBCore.Functions.Notify("DU VANDT EN BIL! OMG!!! En fed Taipan!", "primary", 3500)
	end, coords, true)
end)